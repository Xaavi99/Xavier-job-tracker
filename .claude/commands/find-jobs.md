...
**`europe`** (this branch — Denmark focus): read `DENMARK-STRATEGY.md` first. Target roles from `profile/background.md`'s `industry`/`europe` section, location Denmark only on this branch. Search **Ørsted, Vestas, Siemens Gamesa, Vattenfall, RWE Offshore Wind, Global Wind Service, and Ramboll** career pages directly, plus [JobsInEnglish.dk](https://jobsinenglish.dk/category/engineering/) and [Jobindex.dk](https://www.jobboardfinder.com/jobboard-jobindexdk-denmark). This is a volume play — cast wide across all seven employers rather than concentrating on one. If a role clearly requires working Danish at the level advertised, still capture it but flag that in `jd_text` for the fit-scoring step.
