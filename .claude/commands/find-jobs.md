...
**`europe`** (this branch — Norway focus): read `NORWAY-STRATEGY.md` first. Target roles from `profile/background.md`'s `industry`/`europe` section, location Norway only on this branch. **Equinor is the primary target** — search its careers/Workday portal directly for reliability, asset integrity, floating wind, and O&M-planning roles, plus [OffshoreWindJobs.com](https://www.offshorewindjobs.com/) for smaller Norwegian floating-wind O&M/vessel/inspection firms. Also try Finn.no even though its coverage for this niche is unconfirmed. Weight anything touching floating offshore wind (Hywind Tampen or otherwise) highest — it's the direct match to the dissertation. If a role clearly requires working Norwegian at the level advertised, still capture it but flag that in `jd_text` for the fit-scoring step.
